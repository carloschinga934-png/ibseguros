...
<body>
  ...
  <div class="relative flex size-full min-h-screen flex-col bg-white group/design-root admin-content"
      ...
      <div class="px-40 flex flex-1 justify-center py-5">
        <div class="layout-content-container flex flex-col max-w-[960px] flex-1">
          <div class="flex flex-wrap justify-between gap-3 p-4">
            <div>
              <p class="text-[#111518] tracking-light text-[32px] font-bold leading-tight min-w-72">Historial Médico
                Digital</p>
              <p class="text-[#60768a] text-base">Cliente: {{ $cliente->nombre ?? 'Nombre del Cliente' }}</p>
            </div>
            <a href="{{ route('admin.dashboard') }}"
              class="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-[#f0f2f5] text-[#111518] text-sm font-bold leading-normal tracking-[0.015em]">
          ...
          <!-- Pólizas de salud -->
          <h2 class="text-[#111518] text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Pólizas de
            Salud</h2>
          ...
          @forelse($historial->polizas as $poliza)
            <div class="bg-white rounded-xl border border-[#dbe1e6] p-6 mb-6">
              <div class="flex justify-between items-center mb-4">
                ...
                <span
                  class="px-3 py-1 {{ $poliza->estado == 'activo' ? 'bg-green-100 text-green-800' : ($poliza->estado == 'vencido' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800') }} rounded-full text-sm font-medium">{{ ucfirst($poliza->estado) }}</span>
              </div>
              ...
              @if($poliza->atenciones->count() > 0)
                <div class="overflow-x-auto">
                  <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                      <tr>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha
                        </th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Centro
                          médico</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                          Diagnóstico</th>
                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Monto
                        </th>
                      </tr>
                    </thead>
                      ...
                      @foreach($poliza->atenciones->sortByDesc('fecha') as $atencion)
                        <tr>
                          <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900">
                            {{ $atencion->fecha->format('d/m/Y') }}</td>
                          ...
                          <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-900">S/.
                            {{ number_format($atencion->monto, 2) }}</td>
                        </tr>
                      @endforeach
                    </tbody>
                  </table>
          ...
          <div class="flex justify-end px-4 py-4">
            <a href="{{ route('admin.historial-medico.pdf', ['id' => $historial->id]) }}"
              class="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-[#0b80ee] text-white text-sm font-bold leading-normal tracking-[0.015em]">
          ...
          <div class="flex justify-stretch">
            <div class="flex flex-1 gap-3 flex-wrap px-4 py-3 justify-start">
              <a href="{{ route('admin.historial-medico.pdf', ['id' => $id ?? 1]) }}"
              ...
              <button
                ...
                <a href="{{ route('admin.historial-medico.enviar-email', ['id' => $historial->id]) }}"
                  class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-[#f0f2f5] text-[#111518] text-sm font-bold leading-normal tracking-[0.015em]">
  ...
  <div id="success-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div class="bg-white rounded-lg p-6 max-w-sm text-center shadow-lg">
          <h2 class="text-xl font-bold mb-2 text-green-600">¡Éxito!</h2>
          <p class="text-gray-700">{{ session('success') }}</p>
          <button id="close-modal" class="mt-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors">
              Cerrar
          </button>
      </div>
  </div>
